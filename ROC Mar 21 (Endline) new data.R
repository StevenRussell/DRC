...
AUC1 <- function(var1, var2) {
                ...
                #Percent agreement
                col4 <- (t[1,1] + t[2, 2]) / ( t[1,1] + t[1,2] + t[2,1] + t[2,2] )
                ...
                #PPV
                col7 <- posPredValue(dtab[,"i_Q201_examine"], dtab[,"o_Q112_bc"])
                
                #NPV
                col8 <- negPredValue(dtab[,"i_Q201_examine"], dtab[,"o_Q112_bc"])
...
# Adding column names   
  ...
  names(df) <- c("Interview Question", "Observation Question", "N", "Interview (%)", "Observation (%)",
               "Sensitivity of Self-Report", "Specificity of Self-Report", "% agreement", "AUC", "IF")
...
p <-  print(xtable(df, align=replicate(11, "center")), type="html")
